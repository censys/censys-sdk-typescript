 ...
 */
export function tagsAndCommentsCreateComment(
  client: SDKCore,
  request: operations.V3CommentsCreateCommentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.V3CommentsCreateCommentResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V3CommentsCreateCommentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CreateCommentInputBody, {
    explode: true,
  });

  const path = pathToFunc("/v3/comments")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.V3CommentsCreateCommentResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
