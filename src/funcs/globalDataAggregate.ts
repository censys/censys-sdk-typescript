 ...
 */
export function globalDataAggregate(
  client: SDKCore,
  request: operations.V3GlobaldataSearchAggregateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.V3GlobaldataSearchAggregateResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V3GlobaldataSearchAggregateRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.SearchAggregateInputBody, {
    explode: true,
  });

  const path = pathToFunc("/v3/global/search/aggregate")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.V3GlobaldataSearchAggregateResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
