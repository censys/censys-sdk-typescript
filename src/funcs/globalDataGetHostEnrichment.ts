...
export function globalDataGetHostEnrichment(
  ...
  Result<
    operations.V3GlobaldataAssetHostEnrichmentResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V3GlobaldataAssetHostEnrichmentRequest$outboundSchema.parse(
        value,
      ),
  ...
  const pathParams = {
    host_ip: encodeSimple("host_ip", payload.host_ip, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/v3/global/asset/enrichment/host/{host_ip}")(
    pathParams,
  );
  ...
  const headers = new Headers(compactMap({
    Accept: "application/vnd.censys.api.v3.host-enrichment.v1+json",
  }));
  ...
  >(
    M.json(
      200,
      operations.V3GlobaldataAssetHostEnrichmentResponse$inboundSchema,
      {
        ctype: "application/vnd.censys.api.v3.host-enrichment.v1+json",
        hdrs: true,
        key: "Result",
      },
    ),
